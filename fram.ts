...
class Frame {
  ...
  public static write(stream: BinaryStream, value: Array<Frame>): void {
    ...
    for (const frame of value) {
      ...
      stream.writeByte((frame.reliability << 5) | (frame.isFragmented() ? 16 : 0));
...
function readFrameset(stream: BinaryStream): { sequence: number; frames: Frame[] } {
  stream.readUint8();

  const sequence = stream.readUint24(Endianness.Little);
  const frames = Frame.read(stream);

  return { sequence, frames };
}

export { Frame, readFrameset };
